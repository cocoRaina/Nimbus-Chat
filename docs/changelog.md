...
**追修（日记去重从「按日期」改「按内容」）**：初版按「同一天已有日记就拦」太粗暴——凌晨补记昨天、一天写两篇（内容不同）都被误伤。改成归一化后逐字比对内容，只有内容基本相同（重复写同一篇）才拦，返回 `already_written` 让模型回来问用户「要再写一篇吗」，确认后 `force: true` 追加（不再是 `replace` 覆盖）。比对窗口取目标日期 ±1 天，兼顾凌晨把「昨天」写成今天/昨天两种日期。内容不同的多篇同日日记正常放行。
