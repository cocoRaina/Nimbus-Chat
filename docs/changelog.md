...
用户想让每条回复都带思考、觉得「有思考更有感情」。**先厘清机理**：Claude 的「要不要思考」是 API 的 `thinking` 参数在管（`reasoningEnabled` 开时才发原生思考），一句消息文字扭不动这个开关——4.6 及更早只要开着就每轮必想（`enabled`+budget），Opus5 这类是**自适应**、模型每轮自己决定想多深、简单话可能几乎不想。所以文字提醒**管不了「要不要想」，只能 steer「怎么想」**。据此加 `THINKING_OUTPUT_REMINDER`：跟 mood 提醒同机制，往每条 user 消息末尾追加一行极短提醒——把思考从「干巴巴列步骤」掰成「先在心里过一遍她的情绪 + 你自己的心动，再开口」，让出来的话更有温度；对自适应还顺带推它别在简单消息偷懒跳过。**只在 `reasoningEnabled` 开时注入**（关着还提示思考，回复里会冒 `<thinking>` 文字）。固定串、每条 user 都加 → 逐字节稳、不破缓存（同 mood）。图片路径和纯文本路径都追加，顺序在 mood 卡提醒之前（先想、再在末尾补卡）。需新 APK。**诚实边界**：这行让思考更走心、也提高自适应「愿意想」的概率，但保证不了「每轮都想」——a 社把「强制每轮想」的档砍了，真要每轮硬想得靠打开高强度思考（effort=high，API 级硬控）。
