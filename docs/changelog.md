...
**再追修（日记去重逐字比对失效 → 双信号重写检测）**：逐字比对没用——查库发现真重复的两篇（5-24 的 43/45）措辞、标题全不同（模型每次重写都换词），归一化后也对不上，照样入库。数据分析：真重复对 2-字 shingle 重合系数≈0.26、间隔 3 分钟；合理的同日两篇（7-4 的 69/72，不同事、隔 26h）重合≈0.09。改用**双信号判重写**：同一天 且（已有那篇在 3 小时内写过 **或** 内容 2-字 shingle 重合≥0.2）→ 判疑似重写，返回 `already_written` 让模型问用户，确认后 `force: true` 追加。用真实数据验证：43/45=0.282 拦、43/69 与 45/69=0.086 放行，阈值两侧均有余量。凌晨补记/不同日期/同日不同事均正常放行。
