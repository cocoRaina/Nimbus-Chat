...
| 服务端保活其实能用、之前的"stream 路由"是误判(2026-06-17)| 上一条把锅扣给"stream:true/false 路由到不同节点",**错了**。真因有二:① `App.tsx` 写死 `activeProvider==='openrouter'` 才存请求体 → 金瓜瓜用户的 `cache_keepalive_state` 表是空的,cron 扫不到行、根本没在 ping;② 复测时用 `net.http_post`(pg_net/libcurl)直打金瓜瓜,libcurl 对该 relay 有 HTTP/2 framing bug(`Stream error in the HTTP/2 framing layer`)全挂,被误读成"ping 不命中"。改用 Edge Function 的 Deno `fetch` 实测:`cache_read=65909 / cache_create=0 / input=3`,~66k 历史**整段命中**,非流 ping 完全能续命 | ① 触发门改 `isClaudeModel && (provider==='openrouter' \|\| format==='anthropic')`,金瓜瓜也存体;② Edge Function 加 00:00–08:00(北京)安静时段早返回;③ 重启 `pg_cron` job 3(`cron.alter_job(3, active:=true)`,`*/5 * * * *`)。详见 [caching.md §9](caching.md) |
