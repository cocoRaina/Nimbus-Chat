...
>
> 结论:① 幽灵**跟体走、与来源/头/max_tokens 无关**(P3 排除 App 指纹、P5 排除 max_tokens);② **全量缓存命中时照收**(P5 read=37,261 同时 input=27,035——热轮的真实费用被幽灵支配,约 8~9 倍虚高);③ 幽灵是**对 system+messages 可见文本+tools 的另一套分词重计**(删 thinking 物理 -489 幽灵纹丝不动=思考块不进它的计数;删 tools 幽灵 -8,447≈tools 的重计份额);④ 裸 system 合成体不触发(P1-P3),真实体的哪个特征是开关未再烧钱细分(嫌疑:thinking 参数或 messages 内 cache_control 断点),反正**没有可下手的客户端绕法**——去 tools/去 thinking 都只减不除。⑤ 时间线注意:07-22 晚保活 ping(同体+2001)还是老实账(input=1,740),今天同款配方已中招(P5)——camel 的口径在**扩散**,不是我们端的回归。应对:拿这张表找 camel 对质;或切备用预设;`request_debug.sent_est_tokens`(07-23 埋点)持续对账。
>
