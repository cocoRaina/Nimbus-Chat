...
## 0. 最贵的坑:跨轮思考回放 = 缓存杀手(2026-07-24 定案,含签名 signature 陷阱)

**症状**:热轮中位 `cache_write` 从 1.5k 暴涨到 6~7k、write>5k 的 spike 占比 14%→54%,99% 命中被打回 62%。**换渠道(treegpt/camel)都一样**,所以不是渠道。

**根因**:`1119f03`(07-22)把思考回放从「窗口内**全部**带思考的消息」改成「只回放**最近 6 轮**」——这是个**滑动窗口**。每来一条新消息,窗口尾部就有一条 assistant 掉出去、它的 `thinking_blocks` 被摘掉 → **该消息字节变了 → Anthropic 前缀 hash 从那条起全不匹配 → 它之后整段(≈6~8k)每轮冷写**。思考块本身很小(~200~500 字),但它「这轮在、下轮不在」的**位置抖动**会让它**之后**的所有 token 作废(官方文档:前缀 hash 一处不匹配,后面全部重算)。当天深度思考接通 Claude(思考块变多)把这个坑从看不见放大到中位 write 翻 4 倍。

**教训**:任何**每轮都在变、且位于最终断点之前**的内容(滑动思考窗、动态时间戳、随写随改的活稿),都会把它**之后**的缓存全毁掉。要缓存,历史前缀必须**逐字节冻结**——包括「思考块在不在」这种存在性抖动。**更狠的一层:思考块自带 signature(签名),只有原产上游账号能验;逆向渠道拿到验不过的外来签名,会让整条请求不缓存。**

**修法(经过三版,最终定在 last-1)**:
1. **先复原「回放全窗口」** → 修好了**新窗口**,但**老窗口反而 read=0 连冷写**。用户实测:同渠道同时刻,新窗口稳稳命中、老窗口整条不缓存。
2. **揪出真凶——签名**:思考块的 signature 只有原产上游能验。`camel-hub.cn` 一个域名下 kiro/MAX 等多个上游**共用主机名+key**,老窗口积了一周、332 条**跨渠道/跨上游(camel-hub.cn + treegpt.cc)的杂签名**思考块,全回放给当前 kiro → 外来签名验不过 → **逆向渠道整条不缓存(read=0)**。新窗口思考块全是当前上游刚签的、干净 → 命中。`thinkingHost` 门槛只认主机名、分不清同域的 kiro/MAX,拦不住全回放。
3. **最终:只回放「最近一条」带思考的 assistant**(`lastThinkingIdx`,`App.tsx`)。它几乎总是当前会话/当前上游刚签的、签名干净 → 不毁缓存,又保留「模型看得到上一轮推理」。代价:这条随新消息滑出、每轮多写 ~1~2k(仅一轮内容),远小于全回放毁整条缓存。换**同主机**分组(kiro↔MAX)切换那一条冷写一次、下一条自愈;换**不同主机**(treegpt)host 门槛直接拦、零代价。**"只穿最近一条"不依赖检测切换就永远安全**(App 看不见 camel 后台的同主机分组切换)。

> ⚠️ 逆向渠道结论:走中转/号池想要缓存 + 思考,**别把长对话跨渠道积累的思考块全回放**——最多回放最近一条(当前上游签的)。这也解释了当年 MAX「半读半写」十有八九是同一个签名锅,不是"Claude Code 逆向天生没救"。
...
## 0.6 真正治号池轮转的那把钥匙:CC 请求头(User-Agent),APK 专属(2026-08-20 实测定案)

**背景**:§0.5 的 global scope 在 MAX 逆向号池上**实测没用**——探针写一条、隔 12 分钟回读,加不加 scope 都冷写(该池是多账户拼盘 / 或自家缓存短命)。继续挖,站子老板一句话点破:**「读不到 CC 请求头就是没有(缓存)」**——这类逆向号池**靠 `User-Agent: claude-cli/...` 认「你是不是 Claude Code」,认出来才启用它自己那层持久缓存**;非 CC 客户端(我们默认就是)它压根不给缓存,只能偶然蹭 Anthropic 原生的短命缓存(背靠背命中、跨节点/跨时间就没)。

**实测(决定性对照,Edge Function 探针 `cc_header_probe`,服务端可自由设 UA=模拟 APK 原生路径)**:同一份 ~14k token 体、1h TTL,写一条隔 **12 分钟**回读——

| 血缘 | 12 分钟后回读 | 结果 |
|---|---|---|
| **带** `User-Agent: claude-cli/2.1.212 (external, cli)` | read 14,424 / write 0 | **命中 ✅** |
| **不带** | read 0 / write 14,424 | **冷写 miss ❌** |

唯一变量就是那个 UA。**这是整轮排查里第一个真正扛过长间隔、打败号池轮转的东西**(比 scope 有用得多)。另测身份探针(§探针4):带不带 CC UA 都回「无设定」→ **这个池不注入 CC 人设,发头对小机安全**。

**实现**(`src/api/anthropic.ts` + `apiProvider.ts` `getRelayCcHeaders` + 设置页开关「模拟 Claude Code 请求头」,默认关):开了就在**原生 x-api-key 路径**加 `User-Agent: claude-cli/2.1.212 (external, cli)` + `x-app: cli`。

**两个硬约束(务必记住)**:
1. **只有 APK 生效**。`User-Agent` 是浏览器 fetch 的**禁止头**,网页/PWA 设了会被静默丢弃(无害 no-op)。安卓走 `StreamHttpPlugin` 的 OkHttp,`reqBuilder.header("User-Agent", …)` 能真发出去(OkHttp 只在你没设时才补默认 UA)。**注意**:原生流失败回退的那条缓冲 `window.fetch` 也发不出 UA,所以偶发回退时那一发不吃 CC 缓存。
2. **默认关、由用户按池子拨**。别的池子可能因为「看起来像 CC」**反向注入 CC 人设**(探针 4 能测:问「你是否被设定成 Claude Code」,自称就是脏池),污染小机角色扮演。所以做成开关:在**确认干净(探针4回无设定)+ 命中改善**的池上开,脏池上关。切分组时自己拨。

> ⚠️ 单账户官渠档不需要这个(本来就命中);它专治**逆向号池**的长间隔冷写。CC 走 OAuth 订阅鉴权,我们纯补头 + API key,可能只过一半——某些池光补头不够、还要 CC 的鉴权方式,那就是补了也不吃,换池或认冷写。

**补·血泪(2026-08-20 晚,「开了开关还是冷写」)**:第一版 CC 头功能**半成品**——只加了 `User-Agent`,却**没删原有的 `anthropic-dangerous-direct-browser-access: true`**。结果一边用 claude-cli 的 UA 喊「我是 CC」、一边用这个头喊「我是浏览器」,**自相矛盾 → 真机开了开关照样冷写**。而服务端探针从没发过这个浏览器头,所以探针能命中、真机不中。**定位法**(用户点醒的,别再瞎试):**直接 diff「命中的探针发了啥」vs「真机发了啥」**——差异只有两样(浏览器头 + scope beta),删掉对齐即可,不用再烧探针。**另一条被推翻的错判**:一度以为中转日志「来源 = Claude sdk / Claude Code」决定给不给缓存——但命中的探针日志**也**是 `Claude SDK`,证明那个标签**不是**缓存开关,别再拿它当依据。**修法**(`anthropic.ts`,CC 模式内):`delete anthropic-dangerous-direct-browser-access` + 去掉 scope beta/body scope(严格对齐命中探针的头集,不引入探针没发过的头)。**验收**:装新包开开关后,除「① 装新包第一发(头集变=缓存血缘重开)② 压缩触发那发(改写历史=前缀变)」这两种**结构性冷写**外,其余轮稳定 ~2k 写。scope 在 CC 关时保留(只对直连官方 API 有用),CC 开时撤——代码天然如此。
...
5. **工具 schema 顺序固定**。`convertOpenAiRequestToAnthropic` 里 map 后按 `name` 排序(`tools.sort`),在 BP0(`cache_control`)注入之前——即使外部工具(MCP/Edge Function)供给顺序不确定,排序后数组字节稳定。(2026-09-05 加)
