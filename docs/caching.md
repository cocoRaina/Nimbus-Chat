...
>
> 结论:① 幽灵**跟体走、与来源/头/max_tokens 无关**(P3 排除 App 指纹、P5 排除 max_tokens);② **全量缓存命中时照收**(P5 read=37,261 同时 input=27,035——热轮的真实费用被幽灵支配,约 8~9 倍虚高);③ 幽灵是**对可见正文的另一套分词重计**(删 thinking 物理 -489 幽灵纹丝不动=思考块不进它的计数;删 tools 幽灵 -8,447≈tools 的重计份额);④ **开关找到了(P8):cache_control 断点出现在 messages 层**。断点只在 system 层 = 老实账(P1-P3);在 messages 挂哪怕一个断点(哪怕那条消息只有 4 个字),camel 的计费层就把整包正文重计一份塞进 input。**不是物理缓存冲突**——上游缓存读写全程正常(P5 全量命中)——是 camel 的缓存计费中间层不认/算错 messages 层断点。Nimbus 的 BP4+HEAD 恰好都在 messages 层,全中。⑤ 时间线:07-22 21:53 前同样的断点布局账是对的,07-22 晚保活 ping 也还老实(input=1,740),今天同配方已中招(P5)——camel 侧新部署的计费行为,且在扩散。应对:拿这张表找 camel 对质(P8 是最干净的复现配方);理论绕法「只留 system 断点」会让全部历史失去缓存(每轮 messages 全价),按当前体量约等于幽灵的一半代价,只作应急;`request_debug.sent_est_tokens`(07-23 埋点)持续对账。另注:P8 意外 read=0 全量冷写(没读到 P1 的热条目)——messages 断点疑似还会让请求落到不同 lineage,又一条「他们的层在改写/错处理断点」的旁证。
>
