...
**教训**:任何**每轮都在变、且位于最终断点之前**的内容(滑动思考窗、动态时间戳、随写随改的活稿),都会把它**之后**的缓存全毁掉。要缓存,历史前缀必须**逐字节冻结**——包括「思考块在不在」这种存在性抖动。

**修法**:复原 `1119f03` 之前的「**回放全窗口**」(去掉 `thinkingReplayAllowed.has(msgIdx)` 名单,窗口内每条带思考的 assistant 一律回放)。全窗口的成员是稳定的(新消息只往尾部追加,不改早前消息的去留)→ 前缀逐字节稳 → 那 ~14k 思考只在首次写入花 1.25×、之后一路 0.1× 读。**这远比「滑动 6」每轮 8k×(1.25~2×) 冷写便宜**——当年 `1119f03` 想省的 14k 是假省。注:Anthropic 原生 `/v1/messages` 本就要求带回思考块维持推理/工具连续性,所以这是功能需求,不能砍;窗口无限增大由压缩兜底。
