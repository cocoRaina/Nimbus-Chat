...
| 工具 | 说明 |
|------|------|
| `search_memory` | 跨表向量 + 关键词混合检索（RRF + 时间近度）。可选 `table` 限定来源（memory/diary/letter/timeline/snack_post/snack_reply）。**锁定的核心记忆已常驻注入、不必搜**，本工具主要搜未锁定记忆 + 日记/交接信/时间轴/朋友圈 |
...
| 工具 | 说明 |
|------|------|
| `manage_memory` | 对某条记忆（id 来自 search_memory / list_memories）执行：`lock` 锁定（→ 常驻注入）/ `unlock` 解锁 / `update` 修正合并 / `archive` 软删除（移进 `memories_archive`，可后台找回；锁定的不归档）。详见 [memory.md](memory.md) |
...
| `get_device_state` | 查手机电量 / 充电 / 今日总屏幕时长 / Top 5 app 时长。`@capacitor/device` + 自定义 UsageStats plugin。APK 限定；屏幕时间需在系统设置开「使用情况访问权限」。见 [screen-time.md](screen-time.md) |
