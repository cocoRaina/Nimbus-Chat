...
6. **轻声模式**：整句有声帧（norm≥10）的平均 RMS < 22 → 判定轻声。效果三连：语气标签变「情绪·轻声」（系统提示里约定了 TA 要压低声音、EL 可用 [whispers]）、TA 播报音量降到 0.5×、字幕里插一条「轻声模式 · TA 也放低了声音」。PTT 和免提都生效（PTT 录音时挂了一个独立 analyser 采样）
