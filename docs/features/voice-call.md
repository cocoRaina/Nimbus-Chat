...
通话中双方都是"说出来的",所以聊天记录里:
- **你的话** = 语音条(录音 → voice 附件 → VoiceRecordBubble)
- **小机的话** = 也渲染成**语音条**(点播才 TTS 合成,复用 `[voice]` 那套 VoiceBubble)。判定靠 `callTurnIds`:走消息流一遍,`📞 已接通`→`📞 通话结束` 之间的 assistant 消息算"通话回合",整条 `sanitizeForSpeech` 清洗后当一条语音条。这样通话里小机不用写 `[voice]`(整条本来就要读),但历史里不会露出 `[sighs]` 等标签的文字气泡。TTS 没配时 VoiceBubble 自动降级回文字。纯渲染层判定,历史通话也会追溯变成语音条。
...
- **默认开**(getHandsFree 默认 true,除非显式关)+ 停顿 **0.9s** 就自动发(原 1.2s)→ 一来一回更跟手,更像真通话。真·全双工实时(边说边打断、零延迟)要流式 STT/TTS,是大改,没做
