...
1. **过四道闸**（force 除外）：`enabled` → 今天没醒够 4 次 → 到它自定的 `next_wake_at`
   了 → 不在北京 0–8 点 → **她不在场**（`messages` 里 45min 内没有她的消息）。
   任一没过就跳过并改约（在场 → +1h；深夜/超额 → 顺延 9:00）。
...
4. **自决**：凭此刻心境决定 写随笔 / 发一条朋友圈 / 都不做，并自报下次几小时后醒。
5. **落地**：随笔进 `essays`、朋友圈进 `assistant_posts`（**都不写进聊天**）；更新
   `next_wake_at`（2–8h，落安静时段顺延）、`wakes_today`。
