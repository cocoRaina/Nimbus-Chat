...
## 自动注入的"语音规则"（`buildVoiceSystemSection`）

TTS **就绪时**（开启 + key/voice 填好），`src/storage/ttsConfig.ts` 的 `buildVoiceSystemSection()` 会被拼进系统提示（App.tsx，紧挨贴纸段），无需手动贴。当前规则：

- `[voice]` 里"说出口"的话**必须用英文**（不用中文）。
- ElevenLabs 时额外允许**英文语气标签**（`[laughs]`/`[sighs]`/`[whispers]` 等）；**禁止中文标签**（如 `[笑]`，不生效）。
- 正经长内容/列表/代码仍用普通文字（可中文）。

改这条规则改 `buildVoiceSystemSection` 即可。它在单次会话内稳定，不扰动 BP1 prompt 缓存。
