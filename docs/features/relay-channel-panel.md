...
- **不限额密钥**：NewAPI 对无限额度的 token 返回 `hard_limit_usd = 100000000`（一亿）哨兵值，直接减「已用」会算出 `$99999836` 这种垃圾余额。前端 `isUnlimitedBalance()` 识别 ≥ 99999999 的哨兵（含旧后端只回原始数字的情况），余额显示「不限额」、隐藏总额度，但保留真实的「累计已用」。
