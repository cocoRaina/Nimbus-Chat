-- Hybrid retrieval for memory search: vector recall + keyword (ILIKE) recall
-- fused with Reciprocal Rank Fusion (RRF). Same 6-table union and return shape
-- as search_memories; the search_memory edge function passes query_keywords and
-- calls this. The vector floor (min_similarity) gates ONLY the vector list —
-- keyword matches surface even below the floor (the point: exact names/terms a
-- pure-vector search ranks too low).
...
as $function$
  ...
  order by f.rrf desc, s.similarity desc nulls last
  limit match_count;
$function$;
