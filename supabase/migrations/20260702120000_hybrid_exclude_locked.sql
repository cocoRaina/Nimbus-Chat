-- 自动召回（每轮注入 [相关记忆]）不应重复召回已常驻 system prompt 的锁定
-- 记忆（13/52 条锁定，之前每轮都有概率白占召回名额 + 重复 token）。
-- 加 exclude_locked 参数（默认 false，search_memory 工具的显式搜索不受影响，
-- 自动召回路径传 true）。签名变了需 DROP 重建（否则会留下歧义重载）。

DROP FUNCTION IF EXISTS public.search_memories_hybrid(
  vector, text[], integer, text, double precision, text, text[],
  timestamp with time zone, timestamp with time zone
);

CREATE FUNCTION public.search_memories_hybrid(
