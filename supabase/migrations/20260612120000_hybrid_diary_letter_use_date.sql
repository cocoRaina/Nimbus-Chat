-- diaries / handoff_letters 用各自的 `date`（条目真实日期）而不是 created_at
-- （入库时间）来做时间筛选 + 近度加权 —— 与 timeline 用 event_date 一致。
-- 导入/补写的日记 date 才是有意义的时间，created_at 可能是导入当天。
-- 只改了 diaries / letters 两个 union 的时间列（返回列 + WHERE 过滤）。
