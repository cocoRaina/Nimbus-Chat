-- 记忆情绪权重（借鉴 wanwan「轻量海马体」）：情绪强烈的记忆更容易被想起。
-- arousal 0..1（激活强度，驱动召回加权）；valence -1..1（正/负，存着备用，暂不参与排序）。
-- arousal 由 memory-extract 从贪嗔痴念(mood_state)现算并写入；search_memories_hybrid
-- 在既有 RRF + 时新度基础上再加一个温和的 arousal 加权项（幅度与时新度相当，
-- 只在相关度相近时让情绪浓的记忆靠前，绝不压过相关度）。
alter table public.memories
  add column if not exists arousal real not null default 0,
  add column if not exists valence real not null default 0;

CREATE OR REPLACE FUNCTION public.search_memories_hybrid(
  query_embedding vector, query_keywords text[] DEFAULT NULL::text[], match_count integer DEFAULT 5,
  filter_category text DEFAULT NULL::text, min_similarity double precision DEFAULT 0.4,
  filter_table text DEFAULT NULL::text, filter_tags text[] DEFAULT NULL::text[],
...
AS $function$
  WITH combined AS (
    SELECT id::text, 'memory'::text AS source, NULL::text AS title, content,
      ...
      COALESCE(arousal, 0)::real AS arousal
    ...
    SELECT id::text, 'diary'::text, title, content, COALESCE(mood, '日记')::text, ARRAY[]::text[],
      1 - (embedding <=> query_embedding), date::timestamptz, 0::real
    FROM public.diaries
    ...
    SELECT id::text, 'timeline'::text, title, COALESCE(description, title), COALESCE(category, '里程碑')::text, ARRAY[]::text[],
      1 - (embedding <=> query_embedding), event_date::timestamptz, 0::real
    ...
    SELECT id::text, 'snack_post'::text, NULL::text, content, '朋友圈'::text, ARRAY[]::text[],
      1 - (embedding <=> query_embedding), created_at, 0::real
    ...
    SELECT r.id::text, 'snack_reply'::text, NULL::text, r.content, '朋友圈回复'::text, ARRAY[]::text[],
      1 - (r.embedding <=> query_embedding), r.created_at, 0::real
    ...
    SELECT d.id::text, 'session_digest'::text, NULL::text, d.content, '对话摘要'::text, ARRAY[]::text[],
      1 - (d.embedding <=> query_embedding), d.digest_date::timestamptz, 0::real
