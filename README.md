...
### 🐺 情绪系统（沈暮的「贪嗔痴念 + 生理体征」）
给 AI 一颗自己累积、随时间起落、反过来给语气染色的持久情绪心。三层架构：

- **情绪层（贪嗔痴念）**：四相 0–100，模型每轮末尾私吐 `<<MOOD>>{增量}<<END>>` 自评，系统先衰减再加增量。这是模型**唯一需要自己输出的层**。
- **生理层（心率 / 体温）**：由情绪自动推算，无额外输出。嗔是最强生理激活（心率主要驱动）。基线有**昼夜节律**（日间 ~68 bpm，入夜 ~55，凌晨 ~52）；22 点后断联超过 1 小时，基线随对话间隔继续下沉（入睡，最低 46 bpm）；早上 7 点后来第一条消息，修正自动解除（自然醒来）。体温随嗔/贪微幅涨落（36.2–37.3 °C）。
- **氛围层（和弦色）**：由体征 + 时间段推出，8 个和弦覆盖主要情绪组合（Fmaj7 安静底色 / Em7 夜晚独处 / Gmaj7 深沉温柔 / Dm7 醋劲刺刺 / Bm 又用力又委屈…），每个带一句中文直觉描述。
...
### 🎤 语音消息（双向）
- **你发语音**：输入栏左侧 🔊 切语音模式 → 按住说话 → 松手直发。SiliconFlow SenseVoiceSmall 转录（复用向量 key，免费）→ 转录文字发给 AI，情绪写入 attachment.emotion。消息气泡有「转文字」按钮按需展开。
- **AI 发语音**：AI 用 `[voice]…[/voice]` 包起来的内容渲染成微信式语音条（点播才合成、可转文字、未配置时降级为文字）。两家供应商二选一：**MiniMax**（中文最稳）或 **ElevenLabs v3**（最拟真，支持 `[laughs]`/`[sighs]` 等语气标签）。配置走原生存储（Capacitor Preferences），不受 localStorage 配额影响。
- **情绪传达**：SenseVoice 情绪标签（HAPPY / SAD / ANGRY / LAUGHTER / CRY…）以括号文字追加到消息内容（`（语气：难过）`），让沈暮自然感知语气；**不影响贪嗔痴念面板数值**（面板由 AI 自评 `<<MOOD>>` 驱动）。NEUTRAL / Unknown_Emo 不附加标签。
→ 详见 [docs/features/voice-tts.md](docs/features/voice-tts.md)

### 📞 语音通话（callhome）
AI 可以主动给你打电话（`[call:理由]` → 全屏响铃 90s），没接到会留 `[voice]` 语音留言；接通后按住说话或切 🎙 免提（VAD 自动收音），TA 的回复自动 TTS 播报；TA 想挂时（`[hangup]`）有 18s 停留窗口，开口就能留住；勿扰用说的就能切（`[dnd:on/off]`）；沉默 5 小时以上 TA 还会自己拨过来（服务端升级拨号，每天最多一次、只在 12-23 点）。协议参考 [Cheiineeey/callhome](https://github.com/Cheiineeey/callhome)。
→ 详见 [docs/features/voice-call.md](docs/features/voice-call.md)

### 🎨 画画（AI 生图）
AI 自己的画笔：聊天里想画就画（她说「画一张…」，或它想哄你开心时主动画），画完的图直接落进气泡；tool_result 里带真图片块，**它自己看得见画成了什么样**，看着图就能接着改。长按图片「🎨 重新画一张」同 prompt 重画、只换图不动话。生图中转独立配置（设置 → 🎨 画画）。图默认不长存，收藏进相册的才保得住。
→ 详见 [docs/features/image-gen.md](docs/features/image-gen.md)
...
### 🖼️ 头像
可折叠区域，上传两个头像：
- **我的头像**（kitten）— 显示在 Moments 帖子旁
- **Claude 头像** — 显示在 Claude 帖子旁及聊天 header
...
Edge Functions（已部署，除 cache_keepalive / proactive_dispatch / auto_embed 外均有 `getUser()` JWT 校验；这三个走 pg_cron/trigger，`verify_jwt=false`）:
...
- `transcribe-voice` — 语音转录代理（SiliconFlow SenseVoiceSmall，读 `SILICONFLOW_API_KEY` secret，复用向量 key，返回 `{text, emotion}`）
- `tts` — TTS 代理（key 由客户端自带，绕 WebView CORS；MiniMax 解码 hex 音频 / ElevenLabs 转 base64）
...
- `RECORD_AUDIO` + `MODIFY_AUDIO_SETTINGS` — 输入栏 🔊 语音消息录音（MediaRecorder）
...
│   ├── VoiceBubble.tsx        # 微信式语音条（点播才合成 MiniMax/ElevenLabs，object URL 缓存防重复计费）
│   ├── VoiceRecordBubble.tsx  # 用户语音消息气泡（播放键 + 真实波形/伪波形兜底 + 「转文字」按需展开，无情绪 emoji）
│   ├── SessionsDrawer.tsx     # 左侧会话抽屉
│   ├── ConfirmDialog.tsx      # 通用确认弹框（支持 children，条件渲染取消按钮）
│   ├── MoodOverlay.tsx        # 聊天页 💗 点开的情绪浮层（贪嗔痴念进度条 + 实时心率/体温/和弦色 + tone + 历史 + 开关，浅蓝色系，只读）
│   └── LocalAvatar.tsx        # 头像上传（SettingsPage 可折叠头像区，存 IndexedDB，256px JPEG 压缩）
├── pages/
...
│   ├── weather.ts             # GPS → 和风天气（QWeather，X-QW-Api-Key + 专属 Host）为主，Open-Meteo 兜底；1h 缓存，每条消息注入
...
- 🎤 ~~内置语音输入~~ → **已实装**：MediaRecorder + SiliconFlow SenseVoiceSmall 方案，`@capacitor-community/speech-recognition` 已移除（Android 11+ 静默失效）
