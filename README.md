...
向量记忆（memories / diaries / 交接信 / timeline / 朋友圈）+ 结构化数据（经期 / 健康）；每 12 轮自动提取待确认记忆，逐条确认/忽略。
- **核心记忆「锁定=常驻」**：记忆可 🔒 锁定，**锁定的会自动注入进 system prompt 缓存前缀**（AI 永远知道，无需搜索）；未锁定的留作可搜索归档（库里噪音多，只常驻精选的）。
- **Claude 自管理**：`manage_memory`（锁定/解锁/修正合并/归档）+ `list_memories`（只读通览）让 AI 按需整理记忆库。
- **软删除可找回**：归档进 `memories_archive` 表（AI 不读/不搜/不注入）+ `archive_memory` / `restore_memory` RPC，锁定的不会被归档，用户可在 Supabase 后台找回。
...
真正放在**手机桌面**上的 AppWidget（不开 App 也能看），共三个：
- **经期卡**：相位 / 第几天 / 距下次预测，相位天数在原生侧按 UTC 纯日期实时算，跨天自刷。
- **桌宠**：emoji 风格小宠物（已被螃蟹版取代，保留）。
- **组合卡（主打，默认约 4×2、可拖角缩放）**：左 日期 + 经期信息，右 **会动的 Clawd 螃蟹**（随组件大小缩放）——按相位/时段切 6 种状态动画（经期中→躲歇 / 滤泡期→走动 / 排卵期→开心 / 黄体期→待机 / 夜里→睡 / 无记录→静止）；**戳螃蟹随机播 24 个动画之一**，点左侧开 App。
- 实现：RemoteViews + `ViewFlipper` 帧循环（无需 GIF 解码），帧从 [clawd-tank](https://github.com/marciogranzotto/clawd-tank)（MIT）的 slack-emoji 抽出、放独立资源目录 `res-crab/`，署名见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。App 首页加载时把经期数据推给原生 widget（`storage/periodWidget.ts` → `PeriodWidgetPlugin`）。

### 💬 聊天界面交互（LINE 风格）
气泡分组、一条=一气泡（`[NEXT]`）、工具卡片、长按菜单（自动翻转）、复制（原生剪贴板）、离线条、震动、正在输入指示。
...
- **表情包**：你和 AI 共用一套贴纸，用 `[sticker:名字]` 引用。`+ → 🧷 表情` 导入/发送；AI 也会按注入的贴纸列表自己发。详见 [docs/features/chat-ui.md](docs/features/chat-ui.md)
...
```
   ...
   (数据库 + 认证          (LLM 推理)         (sandbox + 智能家居)
    + Edge Functions)
            ...
            │           (proactive_queue / fcm_tokens 为 FCM 遗留，已弃用)
            │
            ...
            │
            └─→ DB functions: search_memories_hybrid (RPC, 向量+关键词 RRF),
                              archive_memory / restore_memory (记忆软删除),
...
- `pg_cron` — 定时任务扩展（FCM 推送已移除、缓存保活 job 已停，目前无活跃 job）
...
- `cache_keepalive` — 中转保活 ping（代码仍在，pg_cron job 已停，见成本优化节）

> FCM 推送（`send_proactive_push` 函数 + `fcm_tokens` 表）已移除——主动消息改走本地通知（`@capacitor/local-notifications`）。函数若仍残留在 Supabase，可在 Dashboard 手动删除。

DB 函数:
- `search_memories_hybrid(query_embedding, query_keywords, filter_table?, ...)` — 跨表混合检索（向量召回 + 关键词 ILIKE 召回，RPC 内用 RRF 融合）
- `archive_memory(id)` / `restore_memory(archive_id)` — 记忆软删除：原子地在 `memories` ⇄ `memories_archive` 间移动（锁定的不归档）
...
```
src/
├── App.tsx                    # 主路由 + sendMessage + 14 个工具循环 + 锁定记忆注入
├── tools/
...
├── ShareReceiverPlugin.java   # 接收系统分享的文本 → 聊天输入框
