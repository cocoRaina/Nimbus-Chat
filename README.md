...
**自发主动消息**（无需预约）：`proactive_dispatch` 在没有定时消息要发时，检查沉默时长——普通对话沉默 1h、或定时主动消息发出后 30min 未回——就用你的真实模型 + 最近 20 条上下文问 AI 要不要主动说话；AI 返回消息则写入，返回 `NO_SEND` 则 30min 冷却，发出后 2h 冷却（出错也冷却 30min 退避）。CST 00:00–08:00 夜间不触发。app 关着时通过原生 **WorkManager 每 ~15min 轮询 `poll_proactive`** 弹本地通知——不依赖 FCM/HMS，华为等无 GMS 机型也能用（需授通知权限 + 电池白名单）。
...
- `proactive_dispatch` — 主动消息服务端兜底派发（pg_cron 每 5min）：① 扫 `proactive_queue` 到点未发的行，原子抢占后写进 `messages`，同步更新 `cache_keepalive_state.body`；② 定时消息未触发时，检查沉默时长（普通对话 1h / 定时主动消息后未回 30min）+ 冷却 + 夜间守卫，用用户真实模型自发问一次"要不要主动说话"，见 [docs/features/proactive.md](docs/features/proactive.md)
