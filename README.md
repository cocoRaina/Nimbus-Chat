...
**自发主动消息**（无需预约）：`proactive_dispatch` 在没有定时消息要发时，检查沉默时长——普通对话沉默 1h、或定时主动消息发出后 30min 未回——就用你的真实模型 + 最近 20 条上下文问 AI 要不要主动说话；AI 返回消息则写入，返回 `NO_SEND` 则 30min 冷却，发出后 2h 冷却。CST 00:00–08:00 夜间不触发。
