...
### 📞 语音通话（callhome）
AI 可以主动给你打电话（`[call:理由]` → 全屏响铃 90s），没接到会留 `[voice]` 语音留言；接通后按住说话、TA 的回复自动 TTS 播报；TA 想挂时（`[hangup]`）有 18s 停留窗口，开口就能留住；勿扰用说的就能切（`[dnd:on/off]`）。协议参考 [Cheiineeey/callhome](https://github.com/Cheiineeey/callhome)。
