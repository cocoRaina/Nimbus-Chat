...
### 🛠️ Claude 工具（共 19 个）
读取（搜记忆 / 交接信 / 网页 / 通览记忆 list_memories / **get_health_status** 随时查健康+经期）、写入（记忆 / 日记 / 交接信 / 时间轴 / 经期 / 健康）、记忆管理（manage_memory：锁定/解锁/修正/归档 + garden_memories：向量扫描近重复对 + check_memory_health：休眠记忆健康检查）、计算调度（代码沙盒 / 主动消息 / 设备状态）、**音乐媒体**（play_music 网易云搜歌放歌 / control_media 暂停换歌，APK 限定）。聊天里显示为可折叠工具卡片。
...
### 🎵 网易云放歌 + 媒体控制（APK 限定）
Claude 能给你**放指定的歌**：`play_music` 走 `netease_search` Edge Function 服务端搜网易云（绕 WebView CORS），用 `orpheus://song?id=xxx` deep link 直接拉起网易云播放首条结果；`control_media` 发媒体键（`AudioManager.dispatchMediaKeyEvent`）控制当前播放（暂停/继续/上一首/下一首，任意 App 都生效）。需手机已装并登录网易云。
→ 详见 [docs/features/tools.md](docs/features/tools.md)
...
### ⚙️ 模型库
- **API Provider 切换** — OpenRouter ↔ 中转站，一键全局切换。两边都走 Anthropic 原生 `/v1/messages` 协议，prompt cache 都有，工具迭代命中率中转 ~100% / OR ~16%（BP1 兜底）
...
### 🧩 上下文压缩
- **压缩开关**（默认 ✅ 开）— 总开关
- **触发比例**（默认 0.65）— 历史 token 占上下文窗口比例超过此值时触发。**模型支持工具时上限自动收紧到 0.35**（避免 ~$1.18/次工具迭代的全价重读）
...
- **首页 Dashboard** `/` — 时钟 + 横向多页 widget 网格（scroll-snap + 圆点指示器）；长按进编辑模式 → 加组件 / 拖动 / 增删页
...
> FCM 推送（`send_proactive_push` 函数 + `fcm_tokens` 表）已移除——主动消息改走本地通知（`@capacitor/local-notifications`）。
...
- **自定义 `MediaControl` plugin** — `control_media` 工具发媒体键（`AudioManager.dispatchMediaKeyEvent`）；`play_music` 经 `@capacitor/app` `openUrl` 拉起网易云 deep link
...
```
src/
├── App.tsx                    # 主路由 + sendMessage + 19 个工具循环 + 锁定记忆注入 + 每日健康快照注入
├── tools/
...
└── MediaControlPlugin.java    # 自定义 plugin：control_media 发媒体键（AudioManager.dispatchMediaKeyEvent）
