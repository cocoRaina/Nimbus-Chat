...
### ⚙️ 模型库
- **API Provider 切换** — OpenRouter ↔ 中转站，一键全局切换。两边都走 Anthropic 原生 `/v1/messages` 协议，prompt cache 都有。工具迭代轮次也命中历史缓存（标 BP1 + 最后一条 user，2026-06 修正，详见 [caching.md](docs/caching.md)）
...
- `cache_keepalive` — 缓存保活 ping（pg_cron 每 5min 触发，但内部 50min 冷却 → 实际约每小时才真打一次中转，单次 ~¥0.07 热读；覆盖 App 被杀后台的场景；00:00–08:00 北京安静时段不 ping。ping 必须原样带上 `thinking`/`budget_tokens` 才命中聊天那条缓存血脉，见 [caching.md §9](docs/caching.md)）
- `proactive_dispatch` — 主动消息服务端兜底派发（pg_cron 每 5min）：扫 `proactive_queue` 到点未发的行，原子抢占后写进 `messages`，app 关着也照写，见 [docs/features/proactive.md](docs/features/proactive.md)
