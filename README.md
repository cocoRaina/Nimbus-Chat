...
### 🌤️ 天气接入
每天**第一条**用户消息自动附带当前天气（地理位置 + Open-Meteo API，无需 key），通过 `message.meta` 持久化，**保持 prompt cache 稳定**。UI 不显示，只塞给 LLM。
...
| 记忆库 | `/memory-vault` | 4 个 tab：记忆 / 日记 / 交接信 / 时间轴，内联编辑 + 分页（20 条/页）+ 锁定 token 预算提示 + 🔒 锁定 |
...
```
src/
├── App.tsx                    # 主路由 + sendMessage + 15 个工具循环 + 锁定记忆注入
├── tools/
