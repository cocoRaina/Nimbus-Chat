...
AI 用 `[voice]…[/voice]` 包起来的内容渲染成微信式语音条（点播才合成、可转文字、未配置时降级为文字）。两家供应商二选一：**MiniMax**（中文最稳）或 **ElevenLabs v3**（最拟真，支持 `[laughs]`/`[sighs]` 等语气标签）。配置走原生存储（Capacitor Preferences），不受 localStorage 配额影响。
