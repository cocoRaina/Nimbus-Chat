...
**自发主动消息**（无需预约）：`proactive_dispatch` 在没有定时消息要发时，检查沉默时长——普通对话沉默 1h、或定时主动消息发出后 30min 未回——就用你的真实模型 + 最近 20 条上下文问 AI 要不要主动说话；AI 返回消息则写入，返回 `NO_SEND` 则 30min 冷却，发出后 2h 冷却（出错也冷却 30min 退避）。CST 00:00–08:00 夜间不触发；**今日门**：当天 08:00 前没有任何消息则不触发，必须等用户自己先说话（同保活 ping 逻辑）。调 AI 时复用 `cache_keepalive_state.body.system`（带 `cache_control` 的 BP1），追加非缓存的自发指令块——保活每 55min 续命、自发在 1h 后触发，缓存必热，省约 20% token 费。app 关着时通过原生 **WorkManager 每 ~15min 轮询 `poll_proactive`** 弹本地通知——不依赖 FCM/HMS，华为等无 GMS 机型也能用（需授通知权限 + 电池白名单）。
...
- **AI 发语音**：AI 用 `[voice]…[/voice]` 包起来的内容渲染成微信式语音条（点播才合成 MiniMax TTS，key 由用户在设置填，绕 WebView CORS）。
...
- `poll_proactive` — 给原生 WorkManager 后台轮询用（APK）：按 `{user_id, since}` 查 `provider='spontaneous'` 的新消息，返回文本让设备弹**本地通知**。不依赖 FCM/HMS，华为等无 GMS 机型也能用；只查自发消息（定时消息已有设备本地通知，不重复）。service role 绕 RLS，单租户用 anon key 过网关
- `proactive_dispatch` — 主动消息服务端兜底派发（pg_cron 每 5min）：① 扫 `proactive_queue` 到点未发的行，原子抢占后写进 `messages`，同步更新 `cache_keepalive_state.body`；② 定时消息未触发时，检查沉默时长（普通对话 1h / 定时主动消息后未回 30min）+ 冷却 + 夜间守卫 + **今日门**（当天需先有用户消息才触发），用用户真实模型自发问一次"要不要主动说话"；调 AI 时复用 `ksConfig.body.system`（BP1 + `cache_control`）+ 追加非缓存自发指令块，命中保活维护的缓存，见 [docs/features/proactive.md](docs/features/proactive.md)
