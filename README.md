...
### 🛠️ Claude 工具（共 17 个）
读取（搜记忆 / 交接信 / 网页 / 通览记忆 list_memories / **get_health_status** 随时查健康+经期）、写入（记忆 / 日记 / 交接信 / 时间轴 / 经期 / 健康）、记忆管理（manage_memory：锁定/解锁/修正/归档 + garden_memories：向量扫描近重复对 + check_memory_health：休眠记忆健康检查）、计算调度（代码沙盒 / 主动消息 / 设备状态）。聊天里显示为可折叠工具卡片。
...
- **健康快照**：查 `health_data`（昨晚睡眠 / 步数）+ `period_tracking`（经期状态），格式如 `昨晚睡了 5.5h（一般），步数 2341；经期进行中`
...
|------|------|------|
| 压缩开关 | ✅ 开 | 总开关 |
| 触发比例 | 0.65 | 历史 token 占上下文窗口比例超过此值时触发。**模型支持工具时上限自动收紧到 0.35**（避免 ~$1.18/次工具迭代的全价重读） |
| 保留最近消息数 | 20 | 压缩时保留最近 N 条不动，只摘要更早的 |
| Summarizer Provider | OpenRouter | 可以让摘要走 OR（便宜模型），聊天走中转 |
| Summarizer Model | `deepseek/deepseek-chat-v3.1` | DeepSeek 中文摘要稳定，OR 上自带 prompt cache |
...
```
src/
├── App.tsx                    # 主路由 + sendMessage + 17 个工具循环 + 锁定记忆注入 + 每日健康快照注入
├── tools/
