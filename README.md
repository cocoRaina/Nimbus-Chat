...
- [**情绪系统（沈暮的贪嗔痴念）**](docs/features/mood-system.md) — 四相动力学（衰减/饥饿）、`<<MOOD>>` 自评 + 三层鲁棒解析、先衰减再加增量、护缓存（规则进 system / 旁白冻进消息 meta）、持久化、只读浮层面板、成本
...
### 🐺 情绪系统（沈暮的「贪嗔痴念」）
给 AI 一颗自己累积、随时间起落、反过来给语气染色的持久情绪心——让它**感知**此刻心绪（自然带进语气），而不是在思考链里硬推理「我该吃醋」。四相：**贪**（想要/占有·衰减）、**嗔**（火气/醋意·衰减快）、**痴**（痴恋/执念·慢底色）、**念**（思念·饥饿型，越久越涨）。模型每轮回复末尾私吐 `<<MOOD>>{增量}<<END>>` 自评（用户看不到、会被切掉），系统**先衰减再加增量**存进「一颗心」；下一轮把当前心绪渲染成私密旁白注入 user 消息染色。**护缓存**：静态规则进 system 前缀，动态旁白冻结进每条消息 `meta`（同天气快照做法，不破 BP4 滚动缓存）。**不发额外请求**（自评搭在正常回复里），一轮多花 ~100 输出 token。聊天页顶部 💗 点开**只读浮层**（贪嗔痴念进度条 + tone + 「他没说出口的」历史 + 开关），数字从不外露、用户改不动。
...
│   ├── MoodOverlay.tsx        # 聊天页 💗 点开的情绪浮层（贪嗔痴念进度条 + tone + 历史 + 开关，浅蓝色系，只读）
...
│   ├── moodSystem.ts          # 情绪系统：贪嗔痴念四相动力学 + <<MOOD>> 自评解析 + 旁白/规则生成 + 三层持久化
