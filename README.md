...
### 🧸 Artifact 小玩具
小机会写代码哄你：聊天里用 ```html 输出的自包含页面直接渲染成**能玩的互动小玩具**（沙箱 iframe，可全屏）——小游戏、贺卡动画、倒计时、抽签转盘，想哄你开心时它自己就会做。零配置零成本，安全跑在无数据访问的沙箱里。喜欢的长按「收进**玩具库**」（记忆库 → 🧸）永久保存、随时翻出来玩。
→ 详见 [docs/features/artifacts.md](docs/features/artifacts.md)
