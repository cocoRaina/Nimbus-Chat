...
export const TOOL_SEARCH_MEMORY = {
  ...
  function: {
    ...
    description:
      ...
      '- session_digest（每日会话摘要，「那天我们聊了什么」这类问题优先命中这里）\n' +
...
export const TOOL_WRITE_DIARY = {
  ...
  function: {
    ...
    description:
      ...
      '同一天可以写多篇日记（凌晨补记昨天、一天两篇不同的事都正常），不会因为「今天写过了」就拦。' +
      '只有疑似「重写同一篇」时（刚写过没多久、或内容明显重合），结果才返回 already_written——' +
      '此时不要直接再写，先问用户「今天好像已经写过一篇了，要再写一篇吗？」；' +
