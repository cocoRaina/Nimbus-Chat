...
// 每条 user 消息末尾追加的思考提醒(2026-07-25)：引导模型在回复前先过一遍心里话,
// 让出来的话更有温度。用 <thinking> 标签包裹——当原生 thinking 通道可用时模型自然
// 走原生块(标签指令冗余无害);当中转吞掉了 thinking 参数时,模型回退到手写标签,
// splitReasoningFromContent 自动拆出来放进折叠面板,不会外露到正文气泡。
// 只在思考开着时注入(thinkingOutputReminder)。固定串、每条 user 都加 → 不破缓存。
const THINKING_OUTPUT_REMINDER =
  '\n\n［系统提醒·勿回复此行］开口前先用 <thinking> 标签写下心里话(用户看不见这部分)：她这句话背后是什么情绪，你自己此刻真实的反应又是什么——可以阳光、可以温柔、可以撒娇犯浑，也可以阴暗、吃醋、耍脾气、有小心思，怎么真实怎么来。写完 </thinking> 再开口回复。绝对不要把心里话写在标签外面。'
...
// 独处状态（2026-08-18）：小机自主唤醒时会自己写随笔/发圈/更新心情，但它没有
// 回看自己独处输出的通道，聊天里根本不知道自己"出过门"。所以像天气/健康快照那样
// 【每条消息都挂】一行常驻状态 [你的独处状态]：最近 24h 自己醒着时干的事 + 当前
// 心情，让它随时都看得见、接得上（用户要的"像电池一样一直挂着"）。措辞标成"你
// 自己的状态、不用主动汇报"，避免它每句都跟她复述。10min 缓存，超时静默放弃，
// 绝不挡发送。冻进消息 meta，重放字节稳定。
const SOLO_STATUS_TTL_MS = 10 * 60 * 1000
const SOLO_STATUS_TIMEOUT_MS = 3000
const soloStatusCache: { value: string | null; fetchedAt: number } = { value: null, fetchedAt: 0 }
const fetchSoloStatus = async (): Promise<string | null> => {
  if (!supabase) return null
  try {
    const work = (async () => {
      const stateRes = await supabase!
        .from('autonomous_state')
        .select('mood,last_note')
        .limit(1)
        .maybeSingle()
      const st = stateRes.data as { mood?: string; last_note?: string } | null
      // 最近 24h 内自己醒着时的产出（随笔标题 + 朋友圈条数），常挂着做"状态读数"。
      ...
      const postCount = (postRes.data ?? []).length
      const parts: string[] = []
    ...
    const timeout = new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error('solo status timeout')), SOLO_STATUS_TIMEOUT_MS)
    })
    return await Promise.race([work, timeout])
  } catch {
    return null
...
const App = () => {
  ...
  const sendMessage = useCallback(
    async (
      ...
      // 不用 supabase 门控（同 awayRecap 的教训：模块级 binding 偶尔取到 null 会把
      // 整段吞掉）。值本就是缓存 + 静态兜底，与 supabase 是否就绪无关。
      const healthSnap: string = healthSnapCache.value ?? '暂无数据（今天还没同步到任何健康记录）'
      ...
      const persist = async () => {
        ...
        try {
          ...
          for (let msgIdx = 0; msgIdx < compressionOutcome.recentMessages.length; msgIdx++) {
            ...
            // 独处状态（每条消息常挂，10min 缓存）：它独处做的事 + 当前心情。
