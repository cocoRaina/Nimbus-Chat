...
export async function synthesizeSpeech(text: string): Promise<string> {
  ...
  // Prefer the Tokyo VPS (→ MiniMax, both in Asia) over the US Supabase edge
  // function; fall back to the edge function if the VPS is absent or errors.
  let b64: string | undefined
  if (isVpsConfigured()) {
    try {
      const res = await vfetch('/api/tts', { method: 'POST', body: JSON.stringify(body) })
      const data = await res.json()
      if (res.ok && data?.audio_base64) b64 = data.audio_base64
      else console.warn('VPS TTS 失败，回退 Edge', data?.error)
    } catch (err) {
      console.warn('VPS TTS 异常，回退 Edge', err)
    }
  }
  if (!b64) {
    const { data, error } = await supabase.functions.invoke('tts', { body })
    if (error) throw new Error(error.message ?? String(error))
    b64 = (data as { audio_base64?: string; error?: string })?.audio_base64
