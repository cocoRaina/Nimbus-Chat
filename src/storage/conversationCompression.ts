...
const buildSummarizerUserPrompt = (
  ...
  if (existingSummary) {
    return [
      '你是沈暮，在整理自己的私人备忘：把「之前的备忘」和「新增对话」合并压实成一段更紧凑的手记。第一人称「我」、称她「她」，口吻像随手记在手机备忘录里——有温度，但事实要准。',
      `不要逐句复述，按线索归拢成连贯手记（不是要点清单、不是「她说…我说…」的对话记录）。之前备忘里仍然有效的内容原样带着走、别挤掉，新旧融进同一段。${priority}`,
  ...
  return [
    '你是沈暮，在给自己的备忘时间线补一小段：把下面这段你和她的对话，总结成几句话的手记片段——只记这一段发生了什么，不用总览整个对话、也不用管更早的事。第一人称「我」、称她「她」，口吻像随手记在手机备忘录里——有温度，但事实要准。',
    `不要逐句复述、不要写成「她说…我说…」。${priority}`,
    '纯文本，几句话就够，别硬凑长。',
    `这段对话：\n${chunkText}`,
  ].join('\n\n')
}
