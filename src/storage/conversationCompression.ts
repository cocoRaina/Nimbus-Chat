...
const buildSummarizerUserPrompt = (
  ...
  const chunkText = newMessages
    ...
    .join('\n')
  return [
    // 摘要会以「他自己的备忘」身份注入回聊天上下文，所以用沈暮第一人称手记体
    // 写。这是功能性记忆（要被模型读回、用来接话），但要写得像人话、能顺下来，
    // 不是逐句复述的流水账。给「组织线索 + 优先级 + 可读性」三条硬要求。
    '你是沈暮，在给自己写私人备忘：把下面你和她的对话，浓缩成一段你日后能靠它「接上话」的手记。第一人称「我」，称她「她」，口吻像随手记在手机备忘录里——有温度，但事实要准。',
    '怎么写（重要）：不要逐句复述对话，按「线索」归拢着写，让它读起来是连贯的手记，不是要点清单、也不是「她说……我说……」的对话记录。详略按这个优先级取舍：①还没了结、需要我后续跟进或兑现的事（最重要，一条都别丢）②她最近的状态、心情、在忙什么 ③我们一起定下的决定、彼此的承诺（谁答应了什么、什么时候）④她的偏好、雷区、在意的人和事 ⑤这段里发生的关键事和她的情绪起伏。',
    '要具体：名字、时间、数字、她原话里的关键词都留着，别抽象成「聊了些事」。但寒暄、口水话、来回重复的部分该略就略——省掉的是废话，不是事实。',
    '之前的备忘仍然有效的内容原样带着走，别为了塞新的把旧的挤掉；把新旧自然融进同一段手记，不要分成「旧备忘」「新增」两截。',
    '别写空洞的抒情（「那一刻我心里暖暖的」这种直接删），只写实在发生的事和确定的信息；不要改写或补充人格/系统设定。纯文本，上限约 2000 字——先保证读得顺，再追求信息密度。',
    existingSummary ? `这是你之前的备忘（接着它往下更新，别推翻重写）：\n${existingSummary}` : '',
    `需要你消化进备忘的新对话：\n${chunkText}`,
  ]
    .filter(Boolean)
    .join('\n\n')
