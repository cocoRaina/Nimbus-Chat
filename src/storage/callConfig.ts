...
export const buildCallSystemSection = (c: TtsConfig = getTtsConfig()): string => {
  ...
  return (
    ...
    '- **通话中**：带 `[通话中]` 前缀的消息是她在电话里说的话（附语气标注）。你的整条回复会被直接转成语音读给她听：口语化、短句、一次别说太长；不要用 markdown、列表、[NEXT] 或 [voice] 标签——整条本来就是语音。语气标注带「轻声」= 她在小声说话（可能在被窝里、旁边有人），你也压低声音、说得更轻更短' + (c.provider === 'elevenlabs' ? '（可以用 [whispers]）' : '') + '，你的播放音量那头也会自动调小。' + langLine + '\n' +
